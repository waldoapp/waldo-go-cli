...
const (
	FlagSetByCobraAnnotation     = "xcmd_annotation_flag_set_by_xcmd"
	CommandDisplayNameAnnotation = "xcmd_annotation_command_display_name"
)
...
type Command struct {
	...
	// xcmd uses default help command.
	helpCommand *Command
	...
	// commandsAreSorted defines, if command slice are already sorted or not.
...
func (c *Command) HelpFunc() func(*Command, []string) {
	...
	return func(c *Command, a []string) {
		...
		// See https://github.com/spf13/xcmd/issues/1002
...
{{.Title}}{{range $cmds}}{{if (or .IsAvailableCommand (eq .Name "help"))}}
  {{rpad .Name .NamePadding }} {{.Short}}{{end}}{{end}}{{end}}

Additional Commands:{{range $cmds}}{{if (or .IsAvailableCommand (eq .Name "help"))}}
...
func (c *Command) ExecuteC() (cmd *Command, err error) {
	...
	// Workaround FAIL with "go test -v" or "xcmd.test -test.v", see #155
	if c.args == nil && filepath.Base(os.Args[0]) != "xcmd.test" {
		args = os.Args[1:]
	}
...
// Concrete example: https://github.com/spf13/xcmd/issues/393#issuecomment-282741924.
